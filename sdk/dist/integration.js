...
function checkedSnapshot(state, result, minimumSlot) {
    ...
    if (result.legs.length !== state.components.length)
        throw new Error('Route adapter did not return one leg per constituent');
    result.legs.forEach((leg, index) => {
        if (!leg.mint.equals(state.components[index].mint))
            throw new Error('Route leg order does not match the basket recipe');
        if (leg.accounts.some(account => account.isSigner))
            throw new Error('A route cannot introduce another signer');
        const addresses = new Set(leg.accounts.map(account => account.pubkey.toBase58()));
        if (!addresses.has(leg.mint.toBase58()) || !addresses.has(state.components[index].vault.toBase58()))
            throw new Error('Route leg is missing its constituent mint or reserve vault');
...
export function quoteBasketBuy(snapshot, grossSol, slippageBps = 100) {
    ...
    const budgets = (composite) => componentAmounts(market, composite, 'buy').map((tokens, index) => ceilDiv(snapshot.legs[index].buyCost(tokens) * (BPS + bps), BPS));
    let lower = 0n;
    while (lower < upper) {
        const middle = (lower + upper + 1n) / 2n;
        if (budgets(middle).reduce((sum, value) => sum + value, 0n) > fee.net)
            upper = middle - 1n;
        else
            lower = middle;
    }
    if (lower === 0n)
        throw new Error('Buy is too small or constituent liquidity is unavailable');
    const output = buyQuote(market, lower), componentBudgets = budgets(lower);
    const minTokens = output.tokens * (BPS - bps) / BPS;
    if (minTokens === 0n || componentBudgets.some(value => value === 0n))
        throw new Error('Buy is below token or SOL precision');
...
export function quoteBasketSell(snapshot, tokens, slippageBps = 100) {
    ...
    if (minNetSol === 0n || componentMinimums.some(value => value === 0n))
        throw new Error('Sale is below minimum SOL precision');
    return { tokens, minNetSol, componentMinimums, expectedProceeds, expectedNetSol: fee.net, fee };
}
function lookupAddresses(instructions) {
...
export async function prepareBasketBuy(input) {
...
export async function buildBasketTransaction(input) {
    ...
    if (await input.connection.getGenesisHash() !== NETWORK_GENESIS)
...
export async function discoverBasketMints(connection) {
    if (await connection.getGenesisHash() !== NETWORK_GENESIS)
        throw new Error('RPC is not Solana mainnet');
...
export async function verifyDeployment(connection) {
    ...
    if (!program || !program.executable || !program.owner.equals(loader) || program.data.length !== 36 || program.data.readUInt32LE(0) !== 2)
        throw new Error('BASKET program account does not match the upgradeable loader');
    if (!new PublicKey(program.data.subarray(4)).equals(new PublicKey(BASKET_PROGRAM_DATA_ADDRESS)))
        throw new Error('Unexpected BASKET ProgramData address');
    if (!programData || programData.executable || !programData.owner.equals(loader) || programData.data.readUInt32LE(0) !== 3 || programData.data.length !== 45 + BASKET_PROGRAM_BYTES)
        throw new Error('Invalid BASKET ProgramData account');
    const digest = new Uint8Array(await crypto.subtle.digest('SHA-256', Uint8Array.from(programData.data.subarray(45))));
    const sha256 = [...digest].map(byte => byte.toString(16).padStart(2, '0')).join('');
    if (sha256 !== BASKET_PROGRAM_SHA256)
        throw new Error('Deployed BASKET bytecode does not match this SDK release');
    const authority = programData.data[12] === 1 ? new PublicKey(programData.data.subarray(13, 45)).toBase58() : null;
    if (authority !== BASKET_UPGRADE_AUTHORITY)
        throw new Error('BASKET upgrade authority changed; review the release before trading');

...
import { AccountRole, address, appendTransactionMessageInstructions, blockhash as kitBlockhash, compileTransaction as compileKitTransaction, createTransactionMessage, getTransactionEncoder, pipe, setTransactionMessageConfig, setTransactionMessageFeePayer, setTransactionMessageLifetimeUsingBlockhash, } from '@solana/kit';
...
export function compileBasketV1Transaction(payer, recentBlockhash, lastValidBlockHeight, instructions) {
    if (!instructions.length)
        throw new Error('No transaction instructions');
    if (bs58.decode(recentBlockhash).length !== 32)
        throw new Error('Invalid transaction blockhash');
    if (!Number.isSafeInteger(lastValidBlockHeight) || lastValidBlockHeight < 1)
        throw new Error('Invalid transaction expiry');
    ...
    if (wireTransaction.length > V1_TRANSACTION_BYTE_LIMIT)
        throw new Error(`This transaction is ${wireTransaction.length} bytes; transaction v1 allows ${V1_TRANSACTION_BYTE_LIMIT}.`);
    return { version: 1, transaction, wireTransaction, messageBase58: bs58.encode(Uint8Array.from(transaction.messageBytes)), bytes: wireTransaction.length, accounts: accounts.size };
}
/** Read-only exact-byte simulation. Call this before opening any wallet prompt. */
export async function prepareBasketV1Transaction(connection, payer, instructions, quoteSlot) {
    if (!Number.isSafeInteger(quoteSlot) || quoteSlot < 0)
        throw new Error('Invalid quote slot');
    if (await connection.getGenesisHash() !== NETWORK_GENESIS)
        throw new Error('RPC network does not match this SDK release');
    const feature = await connection.getAccountInfoAndContext(TX_V1_FEATURE, { commitment: 'confirmed', minContextSlot: quoteSlot });
    if (!feature.value || feature.value.data.length < 9 || feature.value.data[0] !== 1)
        throw new Error('Solana transaction v1 is not active on this network yet');
...
//# sourceMappingURL=transaction-v1.js.map
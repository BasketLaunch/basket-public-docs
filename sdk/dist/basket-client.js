...
export function decodeBasketMarket(address, account) {
    ...
    const components = Array.from({ length: count }, (_, i) => ({ mint: new PublicKey(state.mints[i]), weightBps: state.weights[i], vault: new PublicKey(state.vaults[i]), recipe: amount(BigInt(state.recipe[i].toString())) }));
...
function verifyBasketSupply(state, mintAccount, supplyAddress, supplyAccount) {
    ...
    if (!supplyAddress.equals(basketAddresses(state.mint, state.mint).supplyVault) || !supplyAccount.owner.equals(TOKEN_PROGRAM_ID))
...
export async function readBasketReserve(rpc, mint, minContextSlot = 0) {
    ...
    const components = state.components.map((component, index) => {
        ...
        if (!component.vault.equals(getAssociatedTokenAddressSync(component.mint, router, true, tokenProgram)))
...
function tradeAccounts(mint, trader, traderTokens) {

...
export async function prepareBasketV1Transaction(connection:Pick<Connection,'rpcEndpoint'|'getGenesisHash'|'getLatestBlockhashAndContext'|'getAccountInfoAndContext'>,payer:PublicKey,instructions:TransactionInstruction[],quoteSlot:number){
  ...
  if(payload.error||!Number.isSafeInteger(slot)||slot!<latest.context.slot||payload.result?.value?.err!=null){const detail=JSON.stringify(payload.error||payload.result?.value?.err);if(detail.includes('MaxInstructionTraceLengthExceeded'))throw new Error('This route invokes too many venue instructions for one atomic transaction. Replace a complex PumpSwap, CLMM, or LaunchLab coin, or use fewer coins. Nothing was submitted.');throw new Error(`Transaction-v1 simulation failed: ${detail}.`);}

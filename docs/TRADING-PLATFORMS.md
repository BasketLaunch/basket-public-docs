...
1. Fetch a consistent market snapshot and fresh accounts for each of up to seven constituent mints. Refresh the selected venue after migration; a temporarily unavailable route must show an actionable error, not an invented quote.
...
A direct caller cannot bypass the protocol checks: the program validates the 1–8 state envelope, ordered canonical reserve PDAs, token programs, venue ownership, balance deltas and immutable fee routing, and Solana rolls back any route that exceeds runtime limits. The official SDK accepts at most seven in this release and exact-simulates every combination before a signature.
